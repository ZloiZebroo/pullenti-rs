...
fn main() {
    ...
    let analyzers = ProcessorService::analyzers();
    ...
    let txt = "Система разрабатывается с 2011 года российским программистом Михаилом \
               Жуковым, проживающим в Москве на Красной площади в доме номер один на \
               втором этаже. Конкурентов у него много: Abbyy, Yandex, ООО \"Russian \
               Context Optimizer\" (RCO) и другие компании. Он планирует продать SDK за \
               1.120.000.001,99 (миллиард сто двадцать миллионов один рубль 99 копеек) \
               рублей, без НДС.";
    println!("\n── Text ──────────────────────────────────────────────────────────────");
    println!("{}", txt);
