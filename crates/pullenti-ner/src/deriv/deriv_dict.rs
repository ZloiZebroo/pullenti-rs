...
use pullenti_morph::{LanguageHelper, MorphLang};
...
impl DerivateDictionary {
    ...
    pub fn find(
        ...
        if ch0 == 'О' || (ch0 == 'И' && ch1 == 'К') {
            ...
            if let Some(r) = self.find_in_trie(&(stem.clone() + "ИЙ"), lang) {
                return Some(r);
            }
            if let Some(r) = self.find_in_trie(&(stem.clone() + "ЫЙ"), lang) {
                return Some(r);
            }
            if ch0 == 'О' && ch1 == 'Н' {
                if let Some(r) = self.find_in_trie(&(stem.clone() + "СКИЙ"), lang) {
                    return Some(r);
                }
            }
            ...
            if let Some(r) = self.find_in_trie(&stem, lang) {
                return Some(r);
            }
        } else if ch0 == 'Е' && ch1 == 'Ь' {
            let stem: String = chars[..n - 2].iter().collect();
            let word1 = stem + "ИЕ";
            if let Some(r) = self.find_in_trie(&word1, lang) {
                return Some(r);
            }
            ...
            if let Some(w1) = word1 {
                if let Some(r) = self.find_in_trie(&w1, lang) {
                    return Some(r);
                }
            }
        }
    ...
    fn find_in_trie(&self, word: &str, lang: MorphLang) -> Option<Vec<&DerivateGroup>> {
        ...
        let mut li: Vec<&DerivateGroup> = ids.iter().filter_map(|&id| self.get_group(id)).collect();
        if li.is_empty() {
            return None;
        }
