...
impl ChemicalToken {
    ...
    pub fn to_formula_string(&self) -> String {
        ...
        if let Some(ref subs) = self.subtokens {
            ...
            for s in subs {
                res.push_str(&s.to_formula_string());
            }
        ...
        if let Some(ref items) = self.items {
            if self.name.is_none() {
                for mnem in items {
                    ...
                    if let Some(c) = chars.next() {
                        res.push(c.to_lowercase().next().unwrap_or(c));
                    }
                }
            }
        }
        if self.num > 0 {
            res.push_str(&self.num.to_string());
        }
        res
    }
}
...
fn try_parse_one(t: &TokenRef, sofa: &SourceOfAnalysis, lev: i32) -> Option<ChemicalToken> {
    ...
    if LanguageHelper::is_hiphen(first_char) && tb.length_char() == 1 {
        ...
        if must_be_name && sub.name.is_none() {
            return None;
        }
    ...
    if is_termin || can_be_part_name(&val_upper) {
        ...
        while let Some(ct) = cur.clone() {
            let cv = {
                let ctb = ct.borrow();
                if !matches!(ctb.kind, TokenKind::Text(_)) {
                    break;
                }
        ...
        if Rc::ptr_eq(t, &end_tok) {
            ...
            if let Some(prev) = t.borrow().prev.as_ref().and_then(|w| w.upgrade()) {
                if prev.borrow().is_value("ФОРМУЛА", None) {
                    ct.is_doubt = false;
                }
            }
        }
            ...
            tb.morph
                ...
                .unwrap_or_else(|| {
                    if let TokenKind::Text(ref td) = tb.kind {
                        ...
                        sofa.substring(tb.begin_char, tb.end_char).to_uppercase()
                    }
                })
        };
        parts.push(word);
        if t.borrow().end_char >= t1_end {
            break;
        }
...
fn can_be_part_name(val: &str) -> bool {
    if val.ends_with("ИД")
        ...
        || val.starts_with("ДИ")
        || val.starts_with("ТРИ")
        || val.starts_with("ЧЕТЫР")
    {
        return true;
    }
    if val.starts_with("ГЕКСА")
        || val.starts_with("ДЕКА")
        || val.starts_with("ТЕТРА")
        || val.starts_with("ПЕНТА")
        || val.starts_with("ГЕПТА")
        || val.starts_with("ОКТА")
        || val.starts_with("НОНА")
        || val.starts_with("УНДЕКА")
        || val.starts_with("ДОДЕКА")
        || val.starts_with("ЭЙКОЗА")
        || val.starts_with("ГЕКТА")
        || val.starts_with("КИЛА")
        || val.starts_with("МИРИА")
    {
        return true;
    }
    if val.ends_with("КИСЬ")
        || val.ends_with("ТОРИД")
        || val.ends_with("ЦЕТАТ")
        || val.ends_with("РАЗИН")
        || val.ends_with("КСИД")
...
fn add_num(ct: &mut ChemicalToken, sofa: &SourceOfAnalysis) {
    let next = match ct.end_token.borrow().next.clone() {
        Some(n) => n,
        None => return,
    };
    if next.borrow().is_whitespace_before(sofa) {
        return;
    }
        ...
        if let Some(num_tok) = next.borrow().next.clone() {
            ...
            if let Some(n) = num_val {
                if let Some(close) = num_tok.borrow().next.clone() {
                    let close_src =
                        sofa.substring(close.borrow().begin_char, close.borrow().end_char);
                    if close_src == ">" || close_src == "]" {
                        ct.num = n;
                        ct.end_token = close;
                    }
...
pub fn create_referent(li: &[ChemicalToken], sofa: &SourceOfAnalysis) -> Option<Referent> {
    if li.len() == 1 {
        ...
        if single_elem || short {
            if !has_chemical_context(&tok.begin_token, sofa) {
                return None;
            }
        }
    }
    ...
    for _ in 0..40 {
        ...
        };
        {
            ...
            if let TokenKind::Referent(ref rd) = pb.kind {
                if rd.referent.borrow().type_name == cr::OBJ_TYPENAME {
                    return true;
                }
            }
