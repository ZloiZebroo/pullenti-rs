...
fn data() -> &'static VacanceData {
    VACANCE_DATA.get_or_init(|| {
        ...
        fn add_variants(
            tc: &mut TerminCollection,
            primary: &str,
            variants: &[&str],
            typ: VacanceTokenType,
            tag2: bool,
        ) {
        ...
        let moral_entries: &[(&str, &[&str])] = &[
            ...
            (
                ...
                &["СТРЕМИТЬСЯ ПОЗНАТЬ НОВОЕ", "СТРЕМЛЕНИЕ УЗНАТЬ НОВОЕ"],
            ),
            (
                "ОБУЧАЕМОСТЬ",
                &[
...
impl VacanceToken {
    ...
    pub fn try_parse_list(
        first_token: &TokenRef,
        sofa: &SourceOfAnalysis,
    ) -> Option<Vec<VacanceToken>> {
        ...
        while let Some(t) = cur {
            ...
            let prev_idx = if adjacent && !res.is_empty() {
                Some(res.len() - 1)
            } else {
                None
            };
        ...
        while i < res.len() {
            ...
            if typ == VacanceTokenType::Expired {
                i += 1;
                continue;
            }
            ...
            if res[i].typ == VacanceTokenType::Undefined && i > 0 && res[i - 1].is_skill() {
                for j in (i + 1)..(i + 2).min(res.len()) {
                    if res[j].is_skill() {
                        let nt = res[j].typ;
                        res[i].typ =
                            if nt == VacanceTokenType::Plus || nt == VacanceTokenType::Moral {
                                nt
                            } else {
                                VacanceTokenType::Skill
                            };
    ...
    fn try_parse_inner(
        ...
        while let Some(tt) = cur.take() {
            ...
            if tt_begin != begin_char && tt.borrow().is_newline_before(sofa) {
                if can_be_start_of_sentence(&tt, sofa) {
                    break;
                }
                ...
                let prev_tok_opt: Option<TokenRef> =
                    tt.borrow().prev.as_ref().and_then(|w| w.upgrade());
                let continues_np = if let Some(ref pt) = prev_tok_opt {
                    let npt = npt_try_parse(pt, NounPhraseParseAttr::No, 0, sofa);
                    npt.as_ref()
                        .map_or(false, |n| n.end_token.borrow().end_char >= tt_begin)
                } else {
                    false
                };
                if !continues_np {
                    let prev_is_noun = prev_tok_opt.as_ref().map_or(false, |p| {
                        p.borrow().get_morph_class_in_dictionary().is_noun()
                    });
            ...
            if let Some(tok) = d.termins.try_parse(&tt) {
                ...
                match ty {
                    ...
                    VacanceTokenType::Language => {
                        ...
                        let mut scan: Option<TokenRef> =
                            tt.borrow().prev.as_ref().and_then(|w| w.upgrade());
                        while let Some(p) = scan.take() {
                            if p.borrow().begin_char < begin_char {
                                break;
                            }
                    ...
                    VacanceTokenType::License => {
                        ...
                        let prev_prev: Option<TokenRef> = tok
                            .end_token
                            .borrow()
                            .prev
                            .as_ref()
            ...
            if let Some(r) = ref_opt {
                ...
                if type_name == "DATE" {
                    let year = r
                        .borrow()
                        .get_string_value("YEAR")
                        .and_then(|s| s.parse::<i32>().ok())
                        .unwrap_or(0);
                    let month = r
                        .borrow()
                        .get_string_value("MONTH")
                        .and_then(|s| s.parse::<i32>().ok())
                        .unwrap_or(0);
                    let day = r
                        .borrow()
                        .get_string_value("DAY")
                        .and_then(|s| s.parse::<i32>().ok())
                        .unwrap_or(0);
            ...
            if let Some(nt) = next {
                ...
                if is_time {
                    ...
                    if let Some(nt2) = nt.borrow().next.clone() {
                        let is_range =
                            nt2.borrow().is_value("ДО", None) || nt2.borrow().is_hiphen(sofa);
                        if is_range {
    ...
    fn try_parse_money(&mut self) -> bool {
        ...
        while let Some(tt) = cur.take() {
            if tt.borrow().begin_char > end_char {
                break;
            }
            if tt.borrow().begin_char - begin_char > 20 {
                break;
            }
            let ref_opt = tt.borrow().get_referent();
            if let Some(r) = ref_opt {
            ...
            if tt.borrow().length_char() <= 3 && tt.borrow().chars.is_letter() {
                ...
                loop {
                        ...
                        && st.borrow().chars.is_letter()
                    {
                        let ch = {
                            let sb = st.borrow();
                            if let TokenKind::Text(ref td) = sb.kind {
                                td.term.clone()
                            } else {
                                String::new()
                            }
    ...
    fn try_parse_education(&mut self) {
        ...
        let mut cur = Some(self.begin_token.clone());
        while let Some(tt) = cur.take() {
            if tt.borrow().end_char > end_char {
                break;
            }
            let v = |s| tt.borrow().is_value(s, None);
            if v("СРЕДНИЙ") || v("СРЕДНЕ") || v("СРЕДН") {
                middl = true;
            } else if v("ВЫСШИЙ") || v("ВЫСШ") {
                hi = true;
            } else if v("ПРОФЕССИОНАЛЬНЫЙ") || v("ПРОФ") || v("ПРОФИЛЬНЫЙ")
            {
                prof = true;
            } else if v("СПЕЦИАЛЬНЫЙ") || v("СПЕЦ") {
                spec = true;
            } else if v("ТЕХНИЧЕСКИЙ") || v("ТЕХ") || v("ТЕХНИЧ") {
                tech = true;
            }
            cur = tt.borrow().next.clone();
        }
    ...
    fn get_value(&mut self, sofa: &SourceOfAnalysis) {
        ...
        while cont {
            ...
            if tb.is_value("ИМЕТЬ", None)
                || tb.is_value("ВЛАДЕТЬ", None)
                || tb.is_value("ЕСТЬ", None)
            {
                if let Some(next) = tb.next.clone() {
                    drop(tb);
                    t0 = next;
