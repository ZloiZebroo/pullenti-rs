...
fn set_str(r: &mut Referent, attr: &str, val: &str) {
    ...
    r.slots
        .push(Slot::new(attr, Some(SlotValue::Str(val.to_string()))));
}

// ── Setters ───────────────────────────────────────────────────────────────────

pub fn set_firstname(r: &mut Referent, v: &str) {
    set_str(r, ATTR_FIRSTNAME, v);
}
pub fn set_middlename(r: &mut Referent, v: &str) {
    set_str(r, ATTR_MIDDLENAME, v);
}
pub fn set_lastname(r: &mut Referent, v: &str) {
    set_str(r, ATTR_LASTNAME, v);
}
pub fn set_sex(r: &mut Referent, v: &str) {
    set_str(r, ATTR_SEX, v);
}
pub fn set_undefname(r: &mut Referent, v: &str) {
    set_str(r, ATTR_UNDEFNAME, v);
}

// ── Getters ───────────────────────────────────────────────────────────────────

pub fn get_firstname(r: &Referent) -> Option<String> {
    get_str(r, ATTR_FIRSTNAME)
}
pub fn get_middlename(r: &Referent) -> Option<String> {
    get_str(r, ATTR_MIDDLENAME)
}
pub fn get_lastname(r: &Referent) -> Option<String> {
    get_str(r, ATTR_LASTNAME)
}
pub fn get_sex(r: &Referent) -> Option<String> {
    get_str(r, ATTR_SEX)
}
...
pub fn to_string_short(r: &Referent) -> String {
    ...
    if let Some(l) = last {
        parts.push(capitalize(&l));
    }
    if let Some(f) = first {
        parts.push(capitalize(&f));
    }
    if let Some(m) = mid {
        parts.push(capitalize(&m));
    }
    if parts.is_empty() {
        if let Some(u) = get_str(r, ATTR_UNDEFNAME) {
            parts.push(capitalize(&u));
        }
    }
    parts.join(" ")
}
