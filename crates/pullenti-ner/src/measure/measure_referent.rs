...
pub fn add_value(r: &mut Referent, v: &str) {
    r.slots
        .push(Slot::new(ATTR_VALUE, Some(SlotValue::Str(v.to_string()))));
}

pub fn set_unit(r: &mut Referent, unit: &str) {
    set_str(r, ATTR_UNIT, unit);
}
pub fn set_kind(r: &mut Referent, kind: &str) {
    set_str(r, ATTR_KIND, kind);
}

pub fn get_value(r: &Referent) -> Option<String> {
    get_str(r, ATTR_VALUE)
}
pub fn get_unit(r: &Referent) -> Option<String> {
    get_str(r, ATTR_UNIT)
}
