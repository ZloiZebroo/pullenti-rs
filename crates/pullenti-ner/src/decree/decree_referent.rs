...
pub fn set_kind(r: &mut Referent, kind: &DecreeKind) {
    ...
    r.slots.push(Slot::new(
        ATTR_KIND,
        Some(SlotValue::Str(kind.as_str().to_string())),
    ));
}

pub fn get_decree_type(r: &Referent) -> Option<String> {
    get_str(r, ATTR_TYPE)
}
pub fn get_decree_name(r: &Referent) -> Option<String> {
    get_str(r, ATTR_NAME)
}
pub fn get_decree_number(r: &Referent) -> Option<String> {
    get_str(r, ATTR_NUMBER)
}
pub fn get_decree_kind(r: &Referent) -> Option<String> {
    get_str(r, ATTR_KIND)
}
