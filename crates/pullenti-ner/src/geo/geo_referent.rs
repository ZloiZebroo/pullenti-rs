...
fn add_str(r: &mut Referent, attr: &str, val: &str) {
    ...
    if r.slots.iter().any(|s| {
        s.type_name == attr
            && s.value
                .as_ref()
                .map(|v| matches!(v, SlotValue::Str(x) if x == &upper))
                .unwrap_or(false)
    }) {
        return;
    }
...
/// ISO alpha-2 code.
pub fn get_alpha2(r: &Referent) -> Option<String> {
    get_str(r, ATTR_ALPHA2)
}
...
fn _is_state_type(v: &str) -> bool {
    v.contains("государство")
        || v.contains("страна")
        || v.contains("держава")
        || v.contains("country")
        || v.contains("империя")
        || v.contains("королевство")
        || v.contains("княжество")
        || v == "союз"
}
